...
### Step 1. The client runs the pose analyzer
...
### Step 2. The client uploads to Claude
...
## Reading the JSON Claude receives
...
## The disclaimer to include once
...
- `visual-aids.md` covers when a picture helps and which tool to use (widget, artifact, or inline markdown). For this skill, default to a form score tile with its band and a table of measured vs target angles, with over and under highlighted.
- `output-formats.md` covers which delivery formats to offer and how. For this skill, offer the chat review (default) and an Artifact page with the annotated key frames and score so they can compare after re-filming. Deliver the chat version first, offer the options in one plain sentence, only offer formats whose tools exist in the session, and remember what the client picks.
