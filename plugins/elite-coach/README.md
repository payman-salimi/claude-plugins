...
1. **Markdown in chat** with a visual alongside it when one helps: warm-up, main block, finisher, cool-down, coaching note, plus a weekly grid or chart.
2. **Excel tracker**: a `.xlsx` to log sets, reps, weights, and RPE week-over-week, generated by `skills/program-design/scripts/build_tracker.py` from a JSON program spec (requires `openpyxl`). Saved under `programs/<client>/` in the working directory by default. The skill then offers the other formats that fit (Google Sheet, Artifact page, Doc, Slack summary) and remembers what the client picks.

## Shared references

Four files in `references/` apply to every skill:

- `training-standards.md` is the single source of truth for the numbers every skill uses: RPE/RIR, weekly volume landmarks, deload prescription, protein and calorie anchors, cardio zones, progression models, and the pain scale. Change a number there first.
- `plain-english.md` is the writing style for everything a client reads: short sentences, everyday words, no em dashes, headlines that read as one plain phrase. If the `plain-english` skill is installed, the skills load it too.
- `visual-aids.md` says when a reply should include a visual (a weekly grid, a trend chart, a decision flowchart, a phase timeline) and which tool to use: an inline widget, an Artifact page, or plain markdown when neither exists.
- `output-formats.md` says which delivery formats to offer for each kind of outcome (Excel, Google Sheet, Artifact, Doc, Slack, Notion, PDF), how to offer them in one sentence, and to remember the client's choice.

Every skill ends with a "Communication and delivery" section that names its default visual and the formats it offers.
